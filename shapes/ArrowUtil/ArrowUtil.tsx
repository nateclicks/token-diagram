...
export class ArrowUtil extends TKShapeUtil<T, E> {
  ...
  Component = TKShapeUtil.Component<T, E, TKMeta>(
    (
      { shape, isEditing, isGhost, meta, events, onShapeChange, onShapeBlur },
      ref
    ) => {
      const {
        id,
      ...
      const labelSize =
        hasLabel || isEditing ? getTextLabelSize(label, font) : [0, 0];
      ...
      const scale = Math.max(
        ...
        Math.min(
          1,
          Math.max(dist / (labelSize[1] + 128), dist / (labelSize[0] + 128))
        )
      );
  ...
  Indicator = TKShapeUtil.Indicator<ArrowShape>(({ shape, bounds }) => {
    const {
      style,
      decorations,
      label,
