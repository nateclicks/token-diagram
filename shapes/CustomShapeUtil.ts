...
import { BINDING_DISTANCE } from '~/state/constants';
import {
  AlignStyle,
  ShapesWithProp,
  TKBinding,
...
export abstract class TKShapeUtil<
  T extends TKShape,
  E extends Element = any
> extends TLShapeUtil<T, E, TKMeta> {
  /* ----------------- Custom Methods ----------------- */
  ...
  getBindingPoint = <K extends TKShape>(
    shape: T,
    fromShape: K,
    point: number[],
    origin: number[],
      ...
      if (Utils.pointInBounds(point, bounds)) {
        ...
        distance = Math.max(
          ...
          Utils.getBoundsSides(bounds)
            .map((side) =>
              Vec.distanceToLineSegment(side[1][0], side[1][1], point)
            )
            .sort((a, b) => a - b)[0]
        );
      }
  ...
  updateChildren?: <K extends TKShape>(
    shape: T,
    children: K[]
  ) => Partial<K>[] | void;

  onChildrenChange?: (shape: T, children: TKShape[]) => Partial<T> | void;
  ...
  getSvgElement = (shape: T, isDarkMode: boolean): SVGElement | void => {
    ...
    if (hasLabel) {
      const s = shape as TKShape & { label: string };
      const g = document.createElementNS('http://www.w3.org/2000/svg', 'g');

      const font = getFontStyle(shape.style);
      const labelSize = getTextLabelSize(shape.label!, font);
      const fontSize =
        getFontSize(shape.style.size, shape.style.font) *
        (shape.style.scale ?? 1);
      ...
      labelElm.setAttribute(
        'transform',
        `translate(${bounds.width / 2 - labelSize[0] / 2}, ${
