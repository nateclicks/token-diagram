...
import { GHOSTED_OPACITY, LETTER_SPACING } from '~/state/constants';
...
export class StickyUtil extends TKShapeUtil<T, E> {
  type = TKShapeType.Sticky as const;
    ...
    (
      ...
      React.useEffect(() => {
        ...
        if (currTextHeight < minTextHeight && size[1] > MIN_CONTAINER_HEIGHT) {
          ...
          onShapeChange?.({
            id: shape.id,
            size: [size[0], MIN_CONTAINER_HEIGHT],
          });
          return;
        }
  ...
  getSvgElement = (shape: T, isDarkMode: boolean): SVGElement | void => {
    ...
    const fontSize =
      getStickyFontSize(shape.style.size) * (shape.style.scale ?? 1);
