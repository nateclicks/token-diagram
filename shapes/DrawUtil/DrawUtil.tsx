...
import { defaultStyle, getShapeStyle } from '~shapes/shared';
import {
  DashStyle,
  DrawShape,
  TKMeta,
...
import {
  getDrawStrokePathTKSnapshot,
  getFillPath,
  getSolidStrokePathTKSnapshot,
} from './drawHelpers';

type T = DrawShape;
type E = SVGSVGElement;

export class DrawUtil extends TKShapeUtil<T, E> {
  type = TKShapeType.Draw as const;
  ...
  Component = TKShapeUtil.Component<T, E, TKMeta>(
    ({ shape, meta, isSelected, isGhost, events }, ref) => {
      ...
      const polygonPathTKSnapshot = React.useMemo(() => {
        return getFillPath(shape);
      }, [points, style.size]);

      const pathTKSnapshot = React.useMemo(() => {
        return style.dash === DashStyle.Draw
          ? getDrawStrokePathTKSnapshot(shape)
          : getSolidStrokePathTKSnapshot(shape);
      ...
      const verySmall =
        bounds.width <= strokeWidth / 2 && bounds.height <= strokeWidth / 2;
  ...
  Indicator = TKShapeUtil.Indicator<T>(({ shape }) => {
    ...
    const pathTKSnapshot = React.useMemo(() => {
      return getSolidStrokePathTKSnapshot(shape);
    }, [points]);
  ...
  hitTestLineSegment = (shape: T, A: number[], B: number[]): boolean => {
    ...
    if (intersectLineSegmentBounds(ptA, ptB, bounds)) {
      for (let i = 1; i < points.length; i++) {
        if (
          intersectLineSegmentLineSegment(points[i - 1], points[i], ptA, ptB)
            .didIntersect
        ) {
          return true;
        }
