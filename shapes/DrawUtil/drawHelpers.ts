 ...
 */
export function getDrawStrokePathTKSnapshot(shape: DrawShape) {
  if (shape.points.length < 2) return '';
 ...
 */
export function getSolidStrokePathTKSnapshot(shape: DrawShape) {
  const { points } = shape;
  if (points.length < 2) return 'M 0 0 L 0 0';
  const options = getFreehandOptions(shape);
  const strokePoints = getDrawStrokePoints(shape, options);
  const last = points[points.length - 1];
  if (!Vec.isEqual(strokePoints[0].point, last))
    strokePoints.push({ point: last } as StrokePoint);
