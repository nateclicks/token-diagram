...
const sortedSelector = (s: TKSnapshot) =>
  Object.values(s.document.pages).sort(
    (a, b) => (a.childIndex || 0) - (b.childIndex || 0)
  );

const currentPageNameSelector = (s: TKSnapshot) =>
  s.document.pages[s.appState.currentPageId].name;

const currentPageIdSelector = (s: TKSnapshot) =>
  s.document.pages[s.appState.currentPageId].id;
...
export function PageMenu() {
  ...
  return (
    <DropdownMenu.Root dir="ltr" open={isOpen} onOpenChange={handleOpenChange}>
      <DropdownMenu.Trigger dir="ltr" asChild id="TK-Page">
...
function PageMenuContent({ onClose }: { onClose: () => void }) {
  ...
  const handleDrag = React.useCallback(
    (ev: React.DragEvent<HTMLDivElement>) => {
      ...
      let dropIndex = sortedPages.findIndex(
        (p) => p.id === ev.currentTarget.id
      );
