...
import { TKAssetType, TKSnapshot } from '~types';

const roomSelector = (state: TKSnapshot) => state.room;
...
export const MultiplayerMenu = function MultiplayerMenu() {
  ...
  const handleCreateMultiplayerProject = React.useCallback(async () => {
    if (app.isDirty) {
      if (app.fileSystemHandle) {
        if (
          window.confirm('Do you want to save changes to your current project?')
        ) {
        ...
        for (const id in nextDocument.assets) {
          ...
          if (asset.src.includes('base64')) {
            const file = dataURLtoFile(
              asset.src,
              asset.fileName ?? asset.type === TKAssetType.Video
                ? 'image.png'
                : 'image.mp4'
            );
  ...
  return (
    <DropdownMenu.Root dir="ltr">
      <DMTriggerIcon id="TK-MultiplayerMenuIcon" isActive={!!room}>
        <MultiplayerIcon2 />
      </DMTriggerIcon>
      <DMContent
        variant="menu"
        id="TK-MultiplayerMenu"
        side="bottom"
        align="start"
        sideOffset={4}
      >
        <DMItem
          id="TK-Multiplayer-CopyInviteLink"
        ...
        <DMItem
          id="TK-Multiplayer-CopyReadOnlyLink"
        ...
        <Divider />
        <DMItem
          id="TK-Multiplayer-CreateMultiplayerProject"
        ...
        <DMItem
          id="TK-Multiplayer-CopyToMultiplayerProject"
