...
import type { TKShape } from '~types';
...
export class TkdrawTestApp extends TkdrawApp {
  ...
  movePointer = (options?: PointerOptions | number[]) => {
    this.onPointerMove(
      inputs.pointerMove(this.getPoint(options), ''),
      {} as React.PointerEvent
    );
    return this;
  };
  ...
  doubleClickCanvas = (options?: PointerOptions | number[]) => {
    ...
    this.onPointerUp(
      inputs.pointerUp(this.getPoint(options), 'canvas'),
      {} as React.PointerEvent
    );
    return this;
  };
  ...
  getPoint(
    options: PointerOptions | number[] = {} as PointerOptions
  ): PointerEvent {
  ...
  expectShapesToHaveProps = <T extends TKShape>(
    shapes: Record<string, Partial<T>>
  ) => {
  ...
  pressEraser = () => {
    this.onPointerDown(
      inputs.pointerDown(this.getPoint(), 'canvas'),
      this.getEraser()
    );
  };
  ...
  releaseEraser = () => {
    this.onPointerUp(
      inputs.pointerUp(this.getPoint(), 'canvas'),
      this.getEraser()
    );
  };
}
