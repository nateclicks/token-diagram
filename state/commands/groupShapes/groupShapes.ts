...
import type { Patch, TKBinding, TkdrawCommand } from '~types';
...
export function groupShapes(
  ...
  const shapesToGroup: TKShape[] = [];
  const deletedGroupIds: string[] = [];
  const otherEffectedGroups: TKShape[] = [];
  ...
  if (
    shapesToGroup.every((shape) => shape.parentId === shapesToGroup[0].parentId)
  ) {
  ...
  const groupBounds = Utils.getCommonBounds(
    shapesToGroup.map((shape) => TLDR.getBounds(shape))
  );

  // Create the group
  beforeShapes[groupId] = undefined;

  afterShapes[groupId] = TLDR.getShapeUtil(TKShapeType.Group).create({
    id: groupId,
  ...
  while (otherEffectedGroups.length > 0) {
    ...
    const nextChildren = (beforeShapes[shape.id]?.children ||
      ...
      (childId) =>
        childId &&
        !(idsToGroup.includes(childId) || deletedGroupIds.includes(childId))
    );
