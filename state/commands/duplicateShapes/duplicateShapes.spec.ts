...
describe('Duplicate command', () => {
  ...
  describe('when duplicating a bound shape', () => {
    it('removed the binding when the target is not selected', () => {
      ...
      const newShapeIds = afterShapeIds.filter(
        (id) => !beforeShapeIds.includes(id)
      );
    ...
    it('duplicates the binding when the target is selected', () => {
      ...
      const newBindingId = app.getShape<ArrowShape>(newShapeIds[0]).handles
        .start.bindingId;
      ...
      expect(
        app.getShape<ArrowShape>(newShapeIds[0]).handles.start.bindingId
      ).toBe(newBindingId);
    });
    ...
    it('duplicates groups', () => {
      ...
      expect(Object.keys(app.page.shapes).length).toBe(
        beforeShapeIds.length + 3
      );
  ...
  it('duplicates in the correct place', () => {
    ...
    expect(
      Utils.getBoundsCenter(
        Utils.getCommonBounds(after.map((shape) => TLDR.getBounds(shape)))
      )
