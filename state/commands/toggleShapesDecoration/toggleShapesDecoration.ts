import type { TkdrawApp } from '~state/internal';
import { Decoration } from '~types';
import type { ArrowShape, Patch, TkdrawCommand } from '~types';
  ...
  const beforeShapes: Record<string, Patch<ArrowShape>> = Object.fromEntries(
    ids.map((id) => [
      id,
      {
        decorations: {
          [decorationId]:
            app.getShape<ArrowShape>(id).decorations?.[decorationId],
        },
      },
    ])
  );
  ...
  const afterShapes: Record<string, Patch<ArrowShape>> = Object.fromEntries(
    ids
      ...
      .map((id) => [
        ...
        {
          decorations: {
            [decorationId]: app.getShape<ArrowShape>(id).decorations?.[
              decorationId
            ]
              ? undefined
              : Decoration.Arrow,
