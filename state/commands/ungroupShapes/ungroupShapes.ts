...
import type { GroupShape, Patch, TKBinding, TKShape } from '~types';
...
export function ungroupShapes(
  ...
  groupShapes
    ...
    .forEach((groupShape) => {
      const shapesToReparent: TKShape[] = [];
      ...
      const endingChildIndex = TLDR.getChildIndexAbove(
        app.state,
        groupShape.id,
        pageId
      );
      ...
      const step =
        (endingChildIndex - startingChildIndex) / shapesToReparent.length;
      ...
      bindings
        .filter(
          (binding) =>
            binding.toId === groupShape.id || binding.fromId === groupShape.id
        )
        .forEach((binding) => {
