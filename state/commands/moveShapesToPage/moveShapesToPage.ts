...
import type { ArrowShape, PagePartial, TKShape, TkdrawCommand } from '~types';
...
export function moveShapesToPage(
  ...
  const shapesToMove = new Set<TKShape>();
  ...
  Object.values(page.bindings)
    .filter(
      (binding) =>
        movingShapeIds.has(binding.fromId) || movingShapeIds.has(binding.toId)
    )
    .forEach((binding) => {
  ...
  const bounds = Utils.getCommonBounds(
    movingShapes.map((shape) => TLDR.getBounds(shape))
  );
