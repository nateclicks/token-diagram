...
export function insertContent(
  ...
  const afterAssets: Record<string, TKAsset> = {};
  ...
  if (overwrite) {
    ...
    const shapesToInsert: TKShape[] = content.shapes
    ...
    const shapesToMove = Object.values(after.shapes) as TKShape[];
    ...
    if (shapesToMove.length > 0) {
      if (point) {
        ...
        const commonBounds = Utils.getCommonBounds(
          shapesToMove.map(TLDR.getBounds)
        );
  ...
  Object.values(after.shapes as Record<string, TKShape>).forEach((shape) => {
    if ('text' in shape) {
