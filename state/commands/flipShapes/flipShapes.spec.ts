...
import { ArrowShape, RectangleShape, TKShapeType } from '~types';
...
describe('Flip command', () => {
  ...
  it('does, undoes and redoes command', () => {
    ...
    expect(app.getShape<RectangleShape>('rect2').point).toStrictEqual([
      100, 100,
    ]);
  ...
  it('flips vertically the children of a grouped shape', () => {
    app.group(
      [
        app.getShape<RectangleShape>('rect1').id,
        app.getShape<RectangleShape>('rect2').id,
      ],
      'groupId'
    );
    app.select('groupId');
  ...
  it('flip curved arrow horizontally', () => {
    ...
    expect(app.getShape<ArrowShape>('arrow1').handles.bend.point).toStrictEqual(
      [214.87, 219.06]
    );
  });
