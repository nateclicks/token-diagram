...
describe('Arrow session', () => {
  ...
  describe('when dragging a bound shape', () => {
    it('updates the arrow', () => {
      ...
      expect(app.bindings.length).toBe(0);
      expect(app.getShape<ArrowShape>('arrow1').point).toStrictEqual([
        200, 200,
      ]);
      expect(
        app.getShape<ArrowShape>('arrow1').handles.start.point
      ).toStrictEqual([0, 0]);
      expect(
        app.getShape<ArrowShape>('arrow1').handles.end.point
      ).toStrictEqual([1, 1]);
    ...
    it.todo(
      'snaps the bend to zero when dragging the bend handle toward the center'
    );
  });
});
...
describe('When creating with an arrow session', () => {
  ...
  it('Creates a start binding if possible', () => {
    ...
    const arrow = app.shapes.find(
      (shape) => shape.type === TKShapeType.Arrow
    ) as ArrowShape;
...
describe('When creating arrows inside of other shapes...', () => {
  it('does not bind an arrow to shapes that contain the whole arrow', () => {
    ...
    expect(arrow.type).toBe(TKShapeType.Arrow);
    expect(app.bindings.length).toBe(0);
    expect(app.shapes.length).toBe(2);
  });
});
