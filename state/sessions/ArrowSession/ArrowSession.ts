...
export class ArrowSession extends BaseSession {
  ...
  constructor(
    ...
    this.bindableShapeIds = TLDR.getBindableShapeIds(app.state).filter(
      (id) =>
        !(id === this.initialShape.id || id === this.initialShape.parentId)
    );
  ...
  update = (): TkdrawPatch | undefined => {
    ...
    const utils = shapeUtils[TKShapeType.Arrow];
  ...
  cancel = (): TkdrawPatch | undefined => {
    ...
    const currentShape = TLDR.onSessionComplete(
    ...
    const afterBindings: Record<string, TKBinding | undefined> = {};
  ...
  complete = (): TkdrawPatch | TkdrawCommand | undefined => {
    ...
    const beforeBindings: Partial<Record<string, TKBinding>> = {};

    const afterBindings: Partial<Record<string, TKBinding>> = {};
  ...
  private findBindingPoint = (
    ...
    const util = TLDR.getShapeUtil<TKShape>(target.type);
