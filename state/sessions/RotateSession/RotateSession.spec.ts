...
describe('Rotate session', () => {
  ...
  describe('when rotating a single shape while pressing shift', () => {
    it('Clamps rotation to 15 degrees', () => {
      ...
      expect(
        Math.round((app.getShape('rect1').rotation || 0) * (180 / Math.PI)) % 15
      ).toEqual(0);
    });
  ...
  describe('when rotating multiple shapes', () => {
    it('keeps the center', () => {
      ...
      app
        .pointBoundsHandle('rotate', { x: 100, y: 0 })
        .movePointer([50, 0])
        .completeSession();
    ...
    it.todo(
      'clears the cached center after any command other than a rotate command, tbh'
    );
