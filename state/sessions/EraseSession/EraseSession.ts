...
export class EraseSession extends BaseSession {
  ...
  status = TKStatus.Creating;
  isLocked?: boolean;
  lockedDirection?: 'horizontal' | 'vertical';
  erasedShapes = new Set<TKShape>();
  erasedBindings = new Set<TKBinding>();
  initialSelectedShapes: TKShape[];
  erasableShapes: Set<TKShape>;
  prevPoint: number[];
        ...
        pages: {
          [page.id]: {
            shapes: Object.fromEntries(
              erasedShapes.map((shape) => [shape.id, { isGhost: true }])
            ),
          },
        },
      },
    ...
    return {
      document: {
        pages: {
          [page.id]: {
            shapes: Object.fromEntries(
              erasedShapes.map((shape) => [shape.id, { isGhost: false }])
            ),
          },
        },
        pageStates: {
