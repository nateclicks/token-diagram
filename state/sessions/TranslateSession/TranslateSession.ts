...
export class TranslateSession extends BaseSession {
  ...
  constructor(
    ...
    const selectedShapes = (
      link
        ? TLDR.getLinkedShapeIds(this.app.state, currentPageId, link, false)
        : selectedIds
    )
    ...
    this.initialCommonBounds = Utils.getCommonBounds(
      this.initialShapes.map(TLDR.getRotatedBounds)
    );
  ...
  start = (): TkdrawPatch | undefined => {
    ...
    const nextBindings: Patch<Record<string, TKBinding>> = {};
    const nextShapes: Patch<Record<string, TKShape>> = {};
  ...
  update = (): TkdrawPatch | undefined => {
    ...
    if (this.isCloning) {
      ...
      if (didChangeCloning) {
        ...
        initialShapes.forEach((shape) => {
          // const current = (nextShapes[shape.id] || this.app.getShape(shape.id)) as TKShape
  ...
  cancel = (): TkdrawPatch | undefined => {
    ...
    const nextBindings: Record<string, Partial<TKBinding> | undefined> = {};
    const nextShapes: Record<string, Partial<TKShape> | undefined> = {};
  ...
  complete = (): TkdrawPatch | TkdrawCommand | undefined => {
    ...
    const beforeBindings: Patch<Record<string, TKBinding>> = {};
    const beforeShapes: Patch<Record<string, TKShape>> = {};

    const afterBindings: Patch<Record<string, TKBinding>> = {};
    const afterShapes: Patch<Record<string, TKShape>> = {};
    ...
    bindingsToDelete.forEach(
      (binding) => (afterBindings[binding.id] = undefined)
    );
  ...
  private createCloneInfo = () => {
    ...
    const clonedBindings: TKBinding[] = [];

    // Create clones of selected shapes
    const clones: TKShape[] = [];
    ...
    initialShapes.forEach((shape) => {
      ...
      const clone = {
        ...
        childIndex: TLDR.getChildIndexAbove(
          this.app.state,
          shape.id,
          currentPageId
        ),
      ...
      if (clone.type === TKShapeType.Video) {
    ...
    Object.values(page.bindings)
      .filter(
        (binding) =>
          clonedShapeIds.has(binding.fromId) || clonedShapeIds.has(binding.toId)
      )
      .forEach((binding) => {
