...
import { CLONING_DISTANCE, DEAD_ZONE } from '~/state/constants';
...
export class SelectTool extends BaseTool<Status> {
  ...
  clonePaint = (point: number[]) => {
    ...
    const gridPoint = [
      center[0] +
        size[0] * Math.floor((point[0] + size[0] / 2 - center[0]) / size[0]),
      center[1] +
        size[1] * Math.floor((point[1] + size[1] / 2 - center[1]) / size[1]),
    ];
      ...
      case 'Tab': {
        ...
        if (
          ...
          if (clonedShape) {
            ...
            this.setStatus(Status.Idle);
            if (clonedShape.type === TKShapeType.Sticky) {
  ...
  onDoubleClickCanvas: TLCanvasEventHandler = () => {
    ...
    // Needs debugging
    // const { currentPoint } = this.app
    // this.app.selectTool(TKShapeType.Text)
    // this.setStatus(Status.Idle)
