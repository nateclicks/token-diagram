...
import type { TokenDocument, TokenFile } from '~types';

const options = { mode: 'readwrite' as const };

const checkPermissions = async (handle: FileSystemFileHandle) => {
  return (
    (await (handle as unknown as FileSystemHandle).queryPermission(options)) ===
      'granted' ||
    (await (handle as unknown as FileSystemHandle).requestPermission(
      options
    )) === 'granted'
  );
};
...
export async function openFromFileSystem(): Promise<null | {
  ...
  // Parse
  const file: TokenFile = JSON.parse(json);
