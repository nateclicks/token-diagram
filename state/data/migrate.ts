...
export function migrate(state: TKSnapshot, newVersion: number): TKSnapshot {
  ...
  Object.values(document.pages).forEach((page) =>
    Object.values(page.shapes).forEach((shape) => {
      ...
      if (shape.type === TKShapeType.Group && children) {
  ...
  if (version !== newVersion) {
    if (version < 14) {
      Object.values(document.pages).forEach((page) => {
        Object.values(page.shapes)
          .filter((shape) => shape.type === TKShapeType.Text)
          .forEach(
            (shape) => (shape as TextShape).style.font === FontStyle.Script
          );
      });
    }
    ...
    if (version <= 13) {
      Object.values(document.pages).forEach((page) => {
        ...
        Object.values(page.shapes).forEach((shape) => {
          ...
          if (shape.type === TKShapeType.Arrow) {
    ...
    Object.values(document.pages).forEach((page) => {
      Object.values(page.shapes).forEach((shape) => {
        if (version < 15.2) {
          if (
            shape.type === TKShapeType.Image ||
            shape.type === TKShapeType.Video
          ) {
        ...
        if (version < 15.3) {
          if (
            shape.type === TKShapeType.Rectangle ||
            shape.type === TKShapeType.Triangle ||
            shape.type === TKShapeType.Ellipse ||
    ...
    if (version < 15.5) {
      settings.exportBackground = TKExportBackground.Transparent;
    }
  }
