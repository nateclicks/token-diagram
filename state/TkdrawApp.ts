...
import { defaultStyle } from '~shapes/shared';
...
import { SessionArgsOfType, TkdrawSession, getSession } from './sessions';
...
export interface TKCallbacks {
  /**
   * (optional) A callback to run when the component mounts.
   */
  onMount?: (app: TkdrawApp) => void;
  /**
   * (optional) A callback to run when the component's state changes.
   */
  onChange?: (app: TkdrawApp, reason?: string) => void;
  /**
   ...
   */
  onSaveProject?: (app: TkdrawApp, e?: KeyboardEvent) => void;
  /**
   ...
   */
  onSaveProjectAs?: (app: TkdrawApp, e?: KeyboardEvent) => void;
  /**
   ...
   */
  onOpenMedia?: (app: TkdrawApp) => void;
  /**
   * (optional) A callback to run when the state is patched.
   */
  onPatch?: (app: TkdrawApp, patch: TkdrawPatch, reason?: string) => void;
  /**
   * (optional) A callback to run when the state is changed with a command.
   */
  onCommand?: (app: TkdrawApp, command: TkdrawCommand, reason?: string) => void;
  /**
   * (optional) A callback to run when the state is persisted.
   */
  onPersist?: (app: TkdrawApp) => void;
  /**
   * (optional) A callback to run when the user undos.
   */
  onUndo?: (app: TkdrawApp) => void;
  /**
   * (optional) A callback to run when the user redos.
   */
  onRedo?: (app: TkdrawApp) => void;
  /**
   ...
   */
  onAssetDelete?: (app: TkdrawApp, assetId: string) => void;
  /**
   ...
   */
  onExport?: (app: TkdrawApp, info: TKExport) => Promise<void>;
  /**
   * (optional) A callback to run when a session begins.
   */
  onSessionStart?: (app: TkdrawApp, id: string) => void;
  /**
   * (optional) A callback to run when a session ends.
   */
  onSessionEnd?: (app: TkdrawApp, id: string) => void;
}

export class TkdrawApp extends StateManager<TKSnapshot> {
  callbacks: TKCallbacks = {};

  tools = {
    select: new SelectTool(this),
    erase: new EraseTool(this),
    [TKShapeType.Text]: new TextTool(this),
    [TKShapeType.Draw]: new DrawTool(this),
    [TKShapeType.Ellipse]: new EllipseTool(this),
    [TKShapeType.Rectangle]: new RectangleTool(this),
    [TKShapeType.Triangle]: new TriangleTool(this),
    [TKShapeType.Line]: new LineTool(this),
    [TKShapeType.Arrow]: new ArrowTool(this),
    [TKShapeType.Sticky]: new StickyTool(this),
  };

  currentTool: BaseTool = this.tools.select;

  session?: TkdrawSession;
  ...
  constructor(id?: string, callbacks = {} as TKCallbacks) {
    super(
   ...
   */
  setSetting = <
    T extends keyof TKSnapshot['settings'],
    V extends TKSnapshot['settings'][T]
  >(
    name: T,
    value: V | ((value: V) => V)
  ): this => {
   ...
   */
  selectTool = (type: TKToolType): this => {
  ...
  resetDocument = (): this => {
    ...
    const doc = TkdrawApp.defaultDocument;
    ...
    this.resetHistory()
      .clearSelectHistory()
      .loadDocument(TkdrawApp.defaultDocument)
      .persist({});
   ...
   * @param document
   */
  updateUsers = (users: TKUser[], isOwnUpdate = false) => {
    this.patchState(
      {
        room: {
   ...
   * @param document
   */
  mergeDocument = (document: TokenDocument): this => {
    ...
    return this.replaceState(
      {
        ...migrate(
          {
            ...this.state,
            document: { ...document, pageStates: currentPageStates },
          },
          TkdrawApp.version
        ),
   ...
   */
  loadDocument = (document: TokenDocument): this => {
  ...
  saveProject = async () => {
    ...
    const fileHandle = await saveToFileSystem(
      migrate(this.state, TkdrawApp.version).document,
  ...
  createPage = (id?: string, name?: string): this => {
    ...
    return this.setState(
      Commands.createPage(this, [-width / 2, -height / 2], id, name)
    );
  };
  ...
  deletePage = (pageId?: string): this => {
    ...
    return this.setState(
      Commands.deletePage(this, pageId ? pageId : this.currentPageId)
    );
  };
   ...
   */
  getAppState = (): TKSnapshot['appState'] => {
   ...
   */
  getPage = (pageId = this.currentPageId): TKPage => {
   ...
   * @param pageId (optional) The page's id.
   */
  getShapes = (pageId = this.currentPageId): TKShape[] => {
   ...
   * @param pageId (optional) The page's id.
   */
  getBindings = (pageId = this.currentPageId): TKBinding[] => {
   ...
   * @param pageId (optional) The page's id.
   */
  getShape = <T extends TKShape = TKShape>(
    id: string,
    pageId = this.currentPageId
  ): T => {
   ...
   * @param pageId (optional) The page's id.
   */
  getBinding = (id: string, pageId = this.currentPageId): TKBinding => {
   ...
   */
  get appState(): TKSnapshot['appState'] {
   ...
   * The current app state.
   */
  get settings(): TKSnapshot['settings'] {
  ...
  paste = async (point?: number[], e?: ClipboardEvent) => {
    ...
    const getSvgFromText = async (text: string) => {
      ...
      const imageBlob = await TLDR.getImageForSvg(svg, TKExportType.SVG, {
        scale: 1,
        quality: 1,
      });
    ...
    const getShapeFromText = (text: string) => {
      const pagePoint = this.getPagePoint(
      ...
      shapesToCreate.push(
        TLDR.getShapeUtil(TKShapeType.Text).getShape({
          id: Utils.uniqueId(),
  ...
  getSvg = async (
    ...
    if (opts.includeFonts) {
      try {
        const { fonts } = await fetch(TkdrawApp.assetSrc, {
          mode: 'no-cors',
        }).then((d) => d.json());
    ...
    const commonBounds = Utils.getCommonBounds(
      shapes.map(TLDR.getRotatedBounds)
    );

    // A quick routine to get an SVG element for each shape
    const getSvgElementForShape = (shape: TKShape) => {
      ...
      if (shape.type === TKShapeType.Image) {
        elm.setAttribute('xlink:href', this.document.assets[shape.assetId].src);
      } else if (shape.type === TKShapeType.Video) {
    ...
    const exportBackground: TKExportBackground = this.settings.exportBackground;
    const darkBackground = '#212529';
    const lightBackground = 'rgb(248, 249, 250)';

    switch (exportBackground) {
      case TKExportBackground.Auto: {
   ...
   * @param content.shapes An array of TKShape objects.
   * @param content.bindings (optional) An array of TKBinding objects.
   * @param content.assets (optional) An array of TKAsset objects.
   ...
   */
  insertContent = (
    content: { shapes: TKShape[]; bindings?: TKBinding[]; assets?: TKAsset[] },
    opts = {} as { point?: number[]; select?: boolean; overwrite?: boolean }
  ) => {
    return this.setState(
      Commands.insertContent(this, content, opts),
      'insert_content'
    );
  };
   ...
   */
  getImage = async (
    format: Exclude<TKExportType, TKExportType.JSON> = TKExportType.PNG,
    opts = {} as Partial<{
      ids: string[];
      scale: number;
      quality: number;
    ...
    const svg = await this.getSvg(ids, {
      includeFonts: format !== TKExportType.SVG,
    });

    if (!svg) return;

    if (format === TKExportType.SVG) {
      const svgString = TLDR.getSvgString(svg, 1);
   ...
   */
  copyImage = async (
    format: TKExportType.PNG | TKExportType.SVG = TKExportType.PNG,
    opts = {} as Partial<{
      ids: string[];
      scale: number;
      quality: number;
   ...
   */
  createShapes = (
    ...shapes: ({ id: string; type: TKShapeType } & Partial<TKShape>)[]
  ): this => {
    if (shapes.length === 0) return this;
   ...
   * @command
   */
  updateShapes = (...shapes: ({ id: string } & Partial<TKShape>)[]): this => {
  ...
  createTextShapeAtPoint(point: number[], id?: string, patch?: boolean): this {
    ...
    const Text = shapeUtils[TKShapeType.Text];
  ...
  getImageOrVideoShapeAtPoint(
    id: string,
    type: TKShapeType.Image | TKShapeType.Video,
    point: number[],
    size: number[],
    assetId: string
  ) {
    ...
    if (size[0] > this.viewport.width) {
      ...
      size[0] =
        this.viewport.width - (FIT_TO_SCREEN_PADDING / this.camera.zoom) * 2;
      size[1] = size[0] * r;
      ...
      size[1] =
        this.viewport.height - (FIT_TO_SCREEN_PADDING / this.camera.zoom) * 2;
      size[0] = size[1] * r;
      if (size[1] < 32 || size[1] < 32) {
        size[0] = 32;
        size[1] = size[0] / r;
   ...
   * @command
   */
  create = (shapes: TKShape[] = [], bindings: TKBinding[] = []): this => {
    if (shapes.length === 0) return this;
   ...
   * @param bindings
   */
  patchCreate = (shapes: TKShape[] = [], bindings: TKBinding[] = []): this => {
    if (shapes.length === 0) return this;
  ...
  moveToPage = (
    ...
    const { rendererBounds } = this;
    this.setState(
      Commands.moveShapesToPage(this, ids, rendererBounds, fromPageId, toPageId)
    );
    return this;
  };
  ...
  nudge = (delta: number[], isMajor = false, ids = this.selectedIds): this => {
    ...
    return this.setState(
      Commands.translateShapes(this, ids, Vec.mul(delta, size))
    );
  };
  ...
  resetBounds = (ids = this.selectedIds): this => {
    ...
    return this.setState(
      Commands.resetBounds(this, ids, this.currentPageId),
      command.id
    );
  };
  ...
  toggleAspectRatioLocked = (ids = this.selectedIds): this => {
    if (ids.length === 0) return this;
    return this.setState(
      Commands.toggleShapeProp(this, ids, 'isAspectRatioLocked')
    );
  };
   ...
   */
  setShapeProps = <T extends TKShape>(
    props: Partial<T>,
    ids = this.selectedIds
  ) => {
  ...
  group = (
    ...
    if (
      ids.length === 1 &&
      this.getShape(ids[0], pageId).type === TKShapeType.Group
    ) {
  ...
  ungroup = (ids = this.selectedIds, pageId = this.currentPageId): this => {
    ...
    const groups = ids
      ...
      .filter((shape) => shape.type === TKShapeType.Group);

    if (groups.length === 0) return this;

    const command = Commands.ungroupShapes(
      this,
      ids,
  ...
  addMediaFromFiles = async (files: File[], point = this.centerPoint) => {
    ...
    for (const file of files) {
      ...
      const shapeType = isImage ? TKShapeType.Image : TKShapeType.Video;
      const assetType = isImage ? TKAssetType.Image : TKAssetType.Video;
      ...
      try {
        ...
        if (typeof src === 'string') {
          ...
          shapesToCreate.push(
            this.getImageOrVideoShapeAtPoint(
              id,
              shapeType,
              point,
              size,
  ...
  onKeyUp: TLKeyboardEventHandler = (key, info, e) => {
    ...
    switch (e.key) {
      case '/': {
        const { currentPoint, shiftKey, metaKey, altKey, ctrlKey, spaceKey } =
          this;
  ...
  onZoom: TLWheelEventHandler = (info, e) => {
    if (this.state.appState.status !== TKStatus.Idle) return;
  ...
  onPointerDown: TLPointerEventHandler = (info, e) => {
    ...
    if (
      this.currentTool.type === TKShapeType.Draw &&
      e.pointerType === 'pen' &&
      e.button === 5
    ) {
  ...
  onPointerUp: TLPointerEventHandler = (info, e) => {
    ...
    if (this.isErasingWithPen && e.pointerType === 'pen' && e.button === 5) {
      this.selectTool(TKShapeType.Draw);
  ...
  onDoubleClickBoundsHandle: TLBoundsHandleEventHandler = (info, e) => {
    ...
    if (shape.type === TKShapeType.Image || shape.type === TKShapeType.Video) {
  ...
  onShapeChange = (shape: { id: string } & Partial<TLShape>) => {
    ...
    const patch = Commands.updateShapes(
      this,
      [shapeToUpdate],
      this.currentPageId
    ).after;
    ...
    if (editingId) {
      ...
      if (shape.type === TKShapeType.Text) {
        if (shape.text.trim().length <= 0) {
          this.patchState(
            Commands.deleteShapes(this, [editingId]).after,
            'delete_empty_text'
          );
  ...
  static defaultDocument: TokenDocument = {
    id: 'doc',
    name: 'New Document',
    version: TkdrawApp.version,
    pages: {
      page: {
        id: 'page',
        name: 'Page 1',
  ...
  static defaultState: TKSnapshot = {
    settings: {
      ...
      language: 'en',
      dockPosition: 'bottom',
      exportBackground: TKExportBackground.Transparent,
    },
    appState: {
      status: TKStatus.Idle,
      activeTool: 'select',
    ...
    document: TkdrawApp.defaultDocument,
  };
