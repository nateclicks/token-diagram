...
import type { TKShapeUtil } from '~shapes/CustomShapeUtil';
import { getTrianglePoints } from '~shapes/TriangleUtil/triangleHelpers';

const isDev = process.env.NODE_ENV === 'development';
export class TLDR {
  static getShapeUtil<T extends TKShape>(type: T['type']): TKShapeUtil<T>;
  static getShapeUtil<T extends TKShape>(shape: T): TKShapeUtil<T>;
  static getShapeUtil<T extends TKShape>(shape: T | T['type']) {
    return getShapeUtil<T>(shape);
  }

  static getSelectedShapes(data: TKSnapshot, pageId: string) {
    const page = TLDR.getPage(data, pageId);
    const selectedIds = TLDR.getSelectedIds(data, pageId);
    return selectedIds.map((id) => page.shapes[id]);
  }

  static screenToWorld(data: TKSnapshot, point: number[]) {
  ...
  static getPage(data: TKSnapshot, pageId: string): TKPage {
    return data.document.pages[pageId];
  }

  static getPageState(data: TKSnapshot, pageId: string): TLPageState {
    return data.document.pageStates[pageId];
  }

  static getSelectedIds(data: TKSnapshot, pageId: string): string[] {
    return TLDR.getPageState(data, pageId).selectedIds;
  }

  static getShapes(data: TKSnapshot, pageId: string): TKShape[] {
    return Object.values(TLDR.getPage(data, pageId).shapes);
  }

  static getCamera(data: TKSnapshot, pageId: string): TLPageState['camera'] {
    return TLDR.getPageState(data, pageId).camera;
  }

  static getShape<T extends TKShape = TKShape>(
    data: TKSnapshot,
    shapeId: string,
    pageId: string
  ): T {
  ...
  static getCenter<T extends TKShape>(shape: T) {
    return TLDR.getShapeUtil(shape).getCenter(shape);
  }

  static getBounds<T extends TKShape>(shape: T) {
    return TLDR.getShapeUtil(shape).getBounds(shape);
  }

  static getRotatedBounds<T extends TKShape>(shape: T) {
    return TLDR.getShapeUtil(shape).getRotatedBounds(shape);
  }

  static getSelectedBounds(data: TKSnapshot): TLBounds {
  ...
  static getParentId(data: TKSnapshot, id: string, pageId: string) {
    return TLDR.getShape(data, id, pageId).parentId;
  }

  // static getPointedId(data: TKSnapshot, id: string, pageId: string): string {
  //   const page = TLDR.getPage(data, pageId)
  ...
  // static getDrilledPointedId(data: TKSnapshot, id: string, pageId: string): string {
  //   const shape = TLDR.getShape(data, id, pageId)
  ...
  // static getTopParentId(data: TKSnapshot, id: string, pageId: string): string {
  //   const page = TLDR.getPage(data, pageId)
  ...
  static getDocumentBranch(
    ...
    return [
      ...
      ...shape.children.flatMap((childId) =>
        TLDR.getDocumentBranch(data, childId, pageId)
      ),
    ];
  }
  ...
  static getSelectedBranchSnapshot(data: TKSnapshot, pageId: string): TKShape[];
  static getSelectedBranchSnapshot<K>(
    data: TKSnapshot,
    pageId: string,
    fn?: (shape: TKShape) => K
    ...
    const copies = TLDR.getSelectedIds(data, pageId)
      .flatMap((id) =>
        TLDR.getDocumentBranch(data, id, pageId).map((id) => page.shapes[id])
      )
  ...
  static getSelectedShapeSnapshot(data: TKSnapshot, pageId: string): TKShape[];
  ...
  static getAllEffectedShapeIds(
    ...
    ids.forEach((id) => {
      ...
      function collectDescendants(shape: TKShape): void {
      ...
      function collectAscendants(shape: TKShape): void {
  ...
  static getBeforeShape<T extends TKShape>(
    shape: T,
    change: Partial<T>
  ): Partial<T> {
  ...
  static mutateShapes<T extends TKShape>(
    data: TKSnapshot,
    ids: string[],
    ...
    ids.forEach((id, i) => {
      ...
      if (
        shape?.type === 'group' &&
        (ids.length === 1 || forceChildrenTraversal)
      ) {
            ...
            shapes: {
              ...Object.fromEntries(
                shapes.flatMap((shape) => {
                  const results: [string, Partial<TKShape> | undefined][] = [
                    [shape.id, undefined],
                  ];
  ...
  static deleteShapes(
    ...
    const shapeIds =
      typeof shapes[0] === 'string'
        ? (shapes as string[])
        : (shapes as TKShape[]).map((shape) => shape.id);
  ...
  static onSessionComplete<T extends TKShape>(shape: T) {
    const delta = TLDR.getShapeUtil(shape).onSessionComplete?.(shape);
    if (!delta) return shape;
    return { ...shape, ...delta };
  }

  static onChildrenChange<T extends TKShape>(
    data: TKSnapshot,
    shape: T,
    pageId: string
  ) {
  ...
  static updateArrowBindings(page: TKPage, arrowShape: ArrowShape) {
    const result = {
          ...
          switch (A.target.type) {
            case TKShapeType.Ellipse: {
              const hits = intersectRayEllipse(
                ...
                (a, b) => Vec.dist(a, B.point) - Vec.dist(b, B.point)
              );
              if (hits[0] !== undefined) {
            ...
            case TKShapeType.Triangle: {
              ...
              const hits = Utils.pointsToLineSegments(points, true)
                .map(([p0, p1]) =>
                  intersectRayLineSegment(B.point, direction, p0, p1)
                )
                  ...
                  Vec.add(
                    ...
                    Vec.mul(
                      ...
                      Math.min(
                        ...
                        BINDING_DISTANCE *
                          2.5 *
                          (Utils.boundsContain(B.bounds, A.intersectBounds)
                            ? -1
                            : 1)
                      )
                    )
    ...
    return TLDR.getShapeUtil<ArrowShape>(arrowShape).onHandleChange?.(
      arrowShape,
      result
    );
  }

  static transform<T extends TKShape>(
    shape: T,
    bounds: TLBounds,
  ...
  static transformSingle<T extends TKShape>(
    shape: T,
    bounds: TLBounds,
    info: TLTransformInfo<T>
  ) {
   ...
   */
  static getRotatedShapeMutation<T extends TKShape>(
  ...
  static getBinding(data: TKSnapshot, id: string, pageId: string): TKBinding {
    return TLDR.getPage(data, pageId).bindings[id];
  }

  static getBindings(data: TKSnapshot, pageId: string): TKBinding[] {
    const page = TLDR.getPage(data, pageId);
    return Object.values(page.bindings);
  }

  static getBindableShapeIds(data: TKSnapshot) {
  ...
  static flattenShape = (data: TKSnapshot, shape: TKShape): TKShape[] => {
    return [
      shape,
      ...(shape.children ?? [])
        .map((childId) =>
          TLDR.getShape(data, childId, data.appState.currentPageId)
        )
        .sort((a, b) => a.childIndex - b.childIndex)
  ...
  static flattenPage = (data: TKSnapshot, pageId: string): TKShape[] => {
    return Object.values(data.document.pages[pageId].shapes)
      .sort((a, b) => a.childIndex - b.childIndex)
      .reduce<TKShape[]>(
        (acc, shape) => [...acc, ...TLDR.flattenShape(data, shape)],
        []
      );
  };

  static getTopChildIndex = (data: TKSnapshot, pageId: string): number => {
  ...
  static assertShapeHasProperty<P extends keyof TKShape>(
    shape: TKShape,
    prop: P
  ...
  static async getImageForSvg(
    svg: SVGElement,
    type: Exclude<TKExportType, TKExportType.JSON> = TKExportType.PNG,
    opts = {} as Partial<{
      scale: number;
      quality: number;
    }>
